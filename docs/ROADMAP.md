...
- [ ] Chess AI — Stockfish adapter и уровни Elo. **IMPLEMENTED → AUTOTESTED → DEPLOYED → gameplay HUMAN TEST PASSED → production-polish spot-check pending.** Stockfish 18 lite single-threaded работает через отдельный Web Worker/WASM adapter; 12 уровней ≈400–2600 Elo; 400–1200 ослабляются через MultiPV + контролируемые ошибки, 1400+ используют UCI_LimitStrength/UCI_Elo. Пользователь подтвердил работу AI и основной партии. Перед финальным `DONE` внедряется production polish: technical piece markers, SAN/figurine move list, captured pieces + material advantage, smooth move animation, approved commander-selection dark surfaces и только синие CTA.
...
## Current phase
**Chess AI — production polish deployment + short HUMAN SPOT-CHECK.** После successful exact-head deploy нужно проверить только новые визуальные/notation изменения. Roster не начинается до финального принятия Chess AI.
...
## CI note
GitHub-hosted Actions всё ещё может завершаться до первого workflow step из-за runner infrastructure и не считается пройденным browser CI. Для Chess AI Cloudflare build command усилен до `npm test && npm run build`, поэтому новый production-polish preview перед передачей пользователю обязан пройти этот exact-head gate. Real Chromium suite обновлён под polish-контракт, но не помечается как исполненный без фактического runner execution.

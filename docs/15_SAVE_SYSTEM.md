...
## Текущее состояние VK / Web — 2026-09-24

- В VK PlatformAdapter читает/пишет облачный компактный envelope с проверкой частей и манифеста; localStorage — локальный кеш/резерв. Прямой запуск GitHub Pages использует локальные данные браузера.
- Контрольные точки Битвы, Стычки и Каравана сохраняют ходы и позволяют возобновлять конкретную позицию. PR #145–147 слиты в `main`.
- Пользователь подтвердил перенос прогресса ноутбук → телефон после ручного восстановления облачного `checksum` 2026-09-24. Причина повреждения и автоматическое восстановление не подтверждены; одновременные активные устройства остаются риском без атомарной записи.
- Метапрогресс Арены пока существует только в плане `features/ARENA_PLAN.md`: его нужно хранить отдельно от состояния похода и согласовывать между устройствами без отката по `journeyStep`.

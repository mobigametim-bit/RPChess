...
const ASSIGNMENT_STATUSES = Object.freeze(['available', 'reserved', 'completed']);
const LEGACY_STATUS_ALIASES = Object.freeze({ released: 'available' });
const CHAIN_ID_ALIASES = Object.freeze({
  'chain.iron_marches.honor': 'chain.iron_marches.honor_of_the_marches'
});
...
function canonicalChainId(value) {
  ...
  return id ? (CHAIN_ID_ALIASES[id] || id) : null;
}
function normalizeAssignment(input) {
  const status = LEGACY_STATUS_ALIASES[input.status] || input.status || 'reserved';
  if (!ASSIGNMENT_STATUSES.includes(status)) throw new Error(`invalid event assignment status: ${status}`);
  return Object.freeze({ nodeId: String(input.nodeId), eventId: String(input.eventId), phase: String(input.phase), status });
}
function createProductionEventSelectorState(libraryInput, options = {}) {
  ...
  return Object.freeze({
    ...
    activeChainIds: freezeArray([...new Set((options.activeChainIds || []).map(canonicalChainId))].sort()),
    history: freezeArray(options.history || [])
  });
}
function assertSelectorState(stateInput, library) {
  if (!stateInput || stateInput.format !== 'rpchess-production-event-selector' || ![1, 2].includes(stateInput.schemaVersion)) throw new Error('invalid production event selector state');
  if (stateInput.libraryId !== library.libraryId) throw new Error('production event selector library mismatch');
  if (stateInput.schemaVersion === 2 && stateInput.assignments.every((entry) => ASSIGNMENT_STATUSES.includes(entry.status))) return stateInput;
  return createProductionEventSelectorState(library, stateInput);
}
function reservationByNode(state) { return new Map(state.assignments.map((entry) => [entry.nodeId, entry])); }
function reservedEventIds(state) { return new Set(state.assignments.filter((entry) => entry.status === 'reserved').map((entry) => entry.eventId)); }
function eventWeight(event, phase, activeChainIds) {
  ...
  const chainId = canonicalChainId(event.chain.id);
  if (event.chain.role === 'followup' && chainId && activeChainIds.has(chainId)) weight *= Math.max(1, Number(event.chain.weightMultiplier || 2));
  return weight;
}
function weightedIndex(entries, seed, salt) {
  ...
  for (let index = 0; index < entries.length; index += 1) { roll -= entries[index].weight; if (roll < 0) return index; }
...
function eligibleEvents(library, state, phase, excludedIds = new Set()) {
  ...
  const activeChains = new Set(state.activeChainIds.map(canonicalChainId));
...
function selectEventForSlot(libraryInput, stateInput, slot, excludedIds = new Set()) {
  ...
  if (existing && existing.status !== 'available') return existing.eventId;
...
function reserveProductionEvents(libraryInput, stateInput, slots = [], options = {}) {
  ...
  for (const slot of slots) {
    ...
    if (existing && existing.status !== 'available') { batchIds.add(existing.eventId); results.push(existing); continue; }
    ...
    if (!eventId) { results.push(null); continue; }
  ...
  const record = Object.freeze({ index: state.history.length, type: 'events_reserved', nodeIds: freezeArray(results.filter(Boolean).map((entry) => entry.nodeId)), eventIds: freezeArray(results.filter(Boolean).map((entry) => entry.eventId)) });
  state = Object.freeze({ ...state, assignments: freezeArray(assignments), history: freezeArray([...state.history, record]) });
...
function releaseProductionEventReservations(libraryInput, stateInput, nodeIds = []) {
  ...
  const assignments = state.assignments.map((entry) => {
    ...
    const next = normalizeAssignment({ ...entry, status: 'available' });
    changed.push(next);
    return next;
  });
  if (!changed.length) return state;
  return Object.freeze({ ...state, assignments: freezeArray(assignments), history: freezeArray([...state.history, Object.freeze({ index: state.history.length, type: 'event_reservations_released', nodeIds: freezeArray(changed.map((entry) => entry.nodeId)), eventIds: freezeArray(changed.map((entry) => entry.eventId)) })]) });
}
function reopenProductionEventReservation(libraryInput, stateInput, nodeId) {
  ...
  let restored = null;
  const assignments = state.assignments.map((entry) => {
    if (entry.nodeId !== target || entry.status !== 'available') return entry;
    restored = normalizeAssignment({ ...entry, status: 'reserved' });
    return restored;
  });
  if (!restored) return state;
  return Object.freeze({ ...state, assignments: freezeArray(assignments), history: freezeArray([...state.history, Object.freeze({ index: state.history.length, type: 'event_reservation_reopened', nodeId: restored.nodeId, eventId: restored.eventId })]) });
}
function completeProductionEventReservation(libraryInput, stateInput, nodeId) {
  ...
  const activeChainIds = new Set(state.activeChainIds.map(canonicalChainId));
  const activatedChainId = event.chain.role === 'start' ? canonicalChainId(event.chain.id) : null;
  if (activatedChainId) activeChainIds.add(activatedChainId);
  return Object.freeze({
    ...state,
    ...
    history: freezeArray([...state.history, Object.freeze({ index: state.history.length, type: 'event_completed', nodeId: completed.nodeId, eventId: completed.eventId, activatedChainId })])
  });
}
function selectorAssignment(stateInput, nodeId) { return stateInput.assignments.find((entry) => entry.nodeId === String(nodeId)) || null; }

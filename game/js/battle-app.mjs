...
function startBattle() {
  ...
  const mode=document.querySelector('[data-game-mode]');if(mode)mode.textContent=`Битва · ${encounter.label} · ${battlePlan.playerColor==='b'?'Вы обороняетесь чёрными':'Вы атакуете белыми'} · ${encounter.tactic}`;
}
...
function leaveAftermath(){audio()?.click?.();resetBattleTracking();showOnly('menu');globalThis.dispatchEvent(new CustomEvent('rpchess:run-continue'));}

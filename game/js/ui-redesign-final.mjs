...
function ensureCss(){if(document.querySelector('[data-ui-redesign-final-css]'))return;const link=document.createElement('link');link.rel='stylesheet';link.href=CSS_HREF;link.dataset.uiRedesignFinalCss='';document.head.append(link);}
...
function activeCombatKind(){if(!desktop()||!visible(classicScreen))return null;if(globalThis.RPChessBattle?.battlePlan)return'battle';if(globalThis.RPChessSkirmish?.battlePlan)return'skirmish';return null;}
function syncCombatBoard(){const kind=activeCombatKind();document.body.classList.toggle('compact-combat-active',Boolean(kind));if(!kind){restore(movePanel,movePanelHome,movePanelNext);return;}const party=classicScreen?.querySelector('.classic-party-panel');if(party&&movePanel&&movePanel.parentElement!==party)party.append(movePanel);}

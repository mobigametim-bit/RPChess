...
function settleCurrentRatedOutcome(run = readRun()) {
  if (!run) return null;
  const puzzle = settlePuzzle(run);
  if (puzzle) return puzzle;
  const battle = settleCombat(run, 'battle');
  const skirmish = settleCombat(run, 'skirmish');
  return battle || skirmish || null;
}
function resultMarkup(receipt) {
  if (!receipt) return '';
  const sign = receipt.delta > 0 ? '+' : '';
  const cls = receipt.delta > 0 ? 'is-positive' : receipt.delta < 0 ? 'is-negative' : '';
  const beforeThreat = threatStarsForPower(receipt.before), afterThreat = threatStarsForPower(receipt.after);
  const threatChanged = beforeThreat !== afterThreat ? `<div class="power-result__threat">УГРОЗА ${starsText(beforeThreat)} → ${starsText(afterThreat)}</div>` : '';
...
function ensureResult(container, before) {
  ...
  if (!root) {
    root = document.createElement('div');root.className='power-result';root.dataset.powerResult='';
    if (before?.parentNode === container) container.insertBefore(root,before); else container.append(root);
  }
  return root;
}
...
function renderCombatResult(run, kind) {
  const last=kind==='skirmish'?run?.lastSkirmish:run?.lastBattle,receiptId=kind==='skirmish'?skirmishReceiptId(run):battleReceiptId(run);
  if(!last||!receiptId)return;const receipt=ratingReceipt(receiptId);if(!receipt)return;
  const selector=kind==='skirmish'?'[data-skirmish-aftermath]':'[data-battle-aftermath]',screen=document.querySelector(selector),panel=screen?.querySelector(kind==='skirmish'?'.skirmish-aftermath-panel':'.battle-aftermath-panel'),button=screen?.querySelector(kind==='skirmish'?'[data-aftermath-continue]':'[data-battle-continue]');
  paint(ensureResult(panel,button),receipt);
}
function renderPuzzleResult(run) {
  const receiptId=puzzleReceiptId(run);if(!receiptId)return;const receipt=ratingReceipt(receiptId),outcome=document.querySelector('[data-puzzle-outcome]'),button=outcome?.querySelector('[data-puzzle-continue]');paint(ensureResult(outcome,button),receipt);
}

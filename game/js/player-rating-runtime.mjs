...
function renderCombatResult(run, kind) {
  ...
  const root = ensureResult(panel, button);
  if (root) { root.innerHTML = resultMarkup(receipt); root.hidden = false; }
}
...
function renderPuzzleResult(run) {
  ...
  const root = ensureResult(outcome, button);
  if (root && receipt) { root.innerHTML = resultMarkup(receipt); root.hidden = false; }
}
...
if (typeof MutationObserver !== 'undefined') {
  new MutationObserver(() => renderPowerResults()).observe(document.querySelector('#app') || document.body, { childList:true, subtree:true, attributes:true, attributeFilter:['hidden'] });
}

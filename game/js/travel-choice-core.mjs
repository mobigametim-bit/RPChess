...
const MECHANICAL_HINTS = Object.freeze({
  ...
  event: 'Исход заранее неизвестен.',
...
const FLAVOR_POOLS = Object.freeze({
  ...
  puzzle: Object.freeze([
    ...
    'Перед отрядом возникает испытание, которое нельзя решить силой.',
    'На каменной плите застыла незавершённая шахматная партия.',
    'Проход закрыт механизмом со знаком древней шахматной задачи.',
    'Чтобы двигаться дальше, придётся найти единственное решение.',
    'У дороги стоит доска с расставленными фигурами и неизвестным условием.',
    'Старинное испытание требует доказать мастерство игры.',
    'Путь охраняет загадка, построенная по законам шахмат.',
    'Каменные фигуры оживают, предлагая решить оставленную позицию.',
    'Перед путниками — короткое испытание, где важен каждый ход.'
  ])
});
...
function isTravelChoice(value) {
  return Boolean(
    ...
    TRAVEL_ENCOUNTER_TYPES.includes(value.type) &&
    Number.isInteger(value.step) && value.step >= 1 &&
    Number.isInteger(value.stars) && value.stars >= 1 && value.stars <= 5 &&
    typeof value.seed === 'string' && value.seed &&
    typeof value.flavor === 'string' && value.flavor &&
    typeof value.mechanicalHint === 'string' && value.mechanicalHint
  );
}
...
function createTravelChoices({ runId, step = 1, types = PLAYABLE_TRAVEL_TYPES } = {}) {
  ...
  const baseThreat = clamp(1 + Math.floor((step - 1) / 2), 1, 5);
  const threatOffsets = shuffled([-1, 0, 1], random);
  const usedFlavorIndexes = new Map();

  return typeSequence.map((type, index) => {
    const stars = clamp(baseThreat + threatOffsets[index], 1, 5);

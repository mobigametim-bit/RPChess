...
async function main(){
  ...
  for(const relative of [
    ...
    'js/skirmish-core.mjs','js/skirmish-app.mjs','js/battle-core.mjs','js/battle-app.mjs','js/battle-route.mjs','js/travel-choice-core.mjs','js/travel-choice-app.mjs',
    'js/resources-core.mjs','js/resources-app.mjs','js/settlement-core.mjs','js/settlement-app.mjs','js/starvation-core.mjs','js/starvation-app.mjs','js/events-data.mjs','js/events-core.mjs','js/events-app.mjs','js/events',
  ...
  for(const relative of [
    'css/events.css','js/events-data.mjs','js/events-core.mjs','js/events-app.mjs','js/encounter-difficulty.mjs','js/race-assets.mjs','js/event-narrative.mjs',
    ...Array.from({length:10},(_,i)=>`js/events/event-data-${String(i+1).padStart(2,'0')}.mjs`),
    'assets/races/humans/pieces/white/king.png','assets/races/humans/pieces/black/king.png','assets/races/orcs/pieces/pawn.png','assets/events/register-04/backgrounds/generic/forest_crossroad.png',
  ...
  ])if(!fs.existsSync(path.join(dist,relative)))throw new Error(`Events build output missing: ${relative}`);
  console.log(`Prepared RPChess Events distribution in ${dist}; Stockfish ${stockfish.version}`);
}

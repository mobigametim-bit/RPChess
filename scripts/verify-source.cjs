...
module.exports=function verifySource(root){
  const required=[
    ...
    'css/reboot-foundation.css','css/classic-chess.css','css/chess-ai-polish.css','css/roster.css','css/skirmish.css','css/battle.css','css/travel-choice.css','css/resources.css','css/settlement.css','css/starvation.css','css/events.css',
    ...
    'js/skirmish-core.mjs','js/skirmish-app.mjs','js/battle-core.mjs','js/battle-app.mjs','js/battle-route.mjs','js/travel-choice-core.mjs','js/travel-choice-app.mjs','js/resources-core.mjs','js/resources-app.mjs','js/settlement-core.mjs','js/settlement-app.mjs','js/starvation-core.mjs','js/starvation-app.mjs','js/events-data.mjs','js/events-core.mjs','js/events-app.mjs',
    'fonts/BrahmsGotischCyr.otf','generated_assets/title_wordmark.png','generated_assets/splash_poster.jpg','generated_assets/node_battle.png','generated_assets/node_elite.png','generated_assets/node_shop.png','generated_assets/node_event.png','generated_assets/reward_gold.png','music/echoes_iron_throne_01.mp3','music/echoes_iron_throne_02.mp3','music/echoes_iron_throne_03.mp3','music/echoes_iron_throne_04.mp3','assets/kings/oathkeeper/portrait.png','assets/kings/oathkeeper/piece.png'
  ];
  ...
  const races=['elves','orcs','undead','dark_elves','dwarves','demons','angels','dragonborn','beastfolk','constructs','animals','fae','goblins'];
  ...
  if(!String(info.version||'').startsWith('3.0.0-events'))fail(`unexpected Events version: ${info.version||'missing'}`);
  if(info.active_feature_branch!=='feature/events')fail(`unexpected active feature branch: ${info.active_feature_branch||'missing'}`);
  ...
  const route=read(root,'js/battle-route.mjs');requireTokens(route,["import './resources-app.mjs'","import './battle-app.mjs'","import './settlement-app.mjs'","import './starvation-app.mjs'","import './events-app.mjs'","import './travel-choice-app.mjs'"],'journey bootstrap');

  const difficulty=read(root,'js/encounter-difficulty.mjs');requireTokens(difficulty,['MAX_ENCOUNTER_STARS = 12','elo: 400','elo: 2600','difficultyForStars'],'12-level difficulty');
  const travel=read(root,'js/travel-choice-core.mjs');requireTokens(travel,['PLAYABLE_TRAVEL_TYPES','skirmish','battle','settlement','event','Array.from({length:TRAVEL_CHOICE_COUNT','MAX_ENCOUNTER_STARS','playerColor','enemyRaceTag'],'Travel equal-pool core');
  const travelApp=read(root,'js/travel-choice-app.mjs');requireTokens(travelApp,['rpchess:event-open','Неделя путешествия','СЛУЧАЙНЫЙ БОЕЦ ПОГИБНЕТ','starsText'],'corrected Travel runtime');
  ...
  for(const cssPath of ['css/roster.css','css/skirmish.css','css/battle.css','css/travel-choice.css','css/resources.css','css/settlement.css','css/starvation.css','css/events.css']){const css=read(root,cssPath);if(css.includes('ui_panel_frame.png')||css.includes('ui_panel_wide.png'))fail(`${cssPath} violates frameless invariant`);}
  return true;
};

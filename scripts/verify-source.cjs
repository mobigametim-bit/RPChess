...
module.exports=function verifySource(root){
  const required=['index.html','BUILD_INFO.json','css/reboot-foundation.css','css/classic-chess.css','css/chess-ai-polish.css','css/roster.css','css/skirmish.css','css/battle.css','css/travel-choice.css','css/player-rating.css','css/resources.css','css/settlement.css','css/starvation.css','css/events.css','css/puzzles.css','css/ux-consistency.css','css/endless-run.css','js/reboot-foundation.mjs','js/reboot-audio.mjs','js/classic-chess-engine.mjs','js/classic-chess-app.mjs','js/chess-ai-adapter.mjs','js/roster-data.mjs','js/run-persistence.mjs','js/roster-app.mjs','js/encounter-difficulty.mjs','js/player-rating.mjs','js/player-rating-runtime.mjs','js/race-assets.mjs','js/event-narrative.mjs','js/endless-run-core.mjs','js/endless-run-app.mjs','js/skirmish-core.mjs','js/skirmish-app.mjs','js/battle-core.mjs','js/battle-app.mjs','js/battle-route.mjs','js/travel-choice-core.mjs','js/travel-choice-app.mjs','js/ux-consistency.mjs','js/resources-core.mjs','js/resources-app.mjs','js/settlement-core.mjs','js/settlement-app.mjs','js/starvation-core.mjs','js/starvation-app.mjs','js/events-data.mjs','js/events-core.mjs','js/events-app.mjs','js/events/event-content-v3.mjs','js/puzzles/puzzle-core.mjs','js/puzzles/puzzle-catalog.mjs','js/puzzles/puzzle-app.mjs','fonts/BrahmsGotischCyr.otf','generated_assets/title_wordmark.png','generated_assets/splash_poster.jpg','generated_assets/node_battle.png','generated_assets/node_elite.png','generated_assets/node_shop.png','generated_assets/node_event.png','generated_assets/node_training.png','generated_assets/reward_gold.png','music/echoes_iron_throne_01.mp3','music/echoes_iron_throne_02.mp3','music/echoes_iron_throne_03.mp3','music/echoes_iron_throne_04.mp3','assets/kings/oathkeeper/portrait.png','assets/kings/oathkeeper/piece.png'];
  ...
  const route=read(root,'js/battle-route.mjs');requireTokens(route,["import './player-rating-runtime.mjs'","import './endless-run-app.mjs'","import './resources-app.mjs'","import './battle-app.mjs'","import './settlement-app.mjs'","import './starvation-app.mjs'","import './events-app.mjs'","import './puzzles/puzzle-app.mjs'","import './travel-choice-app.mjs'","import './ux-consistency.mjs'"],'journey bootstrap');

  const endlessCore=read(root,'js/endless-run-core.mjs');requireTokens(endlessCore,['END_REASON_LABELS','goldEarned','skirmishWins','battleWins','puzzlesSolved','eventsResolved','accrueRunStats','recruitedHeroCount','summarizeRun'],'Endless Run statistics core');
  const endlessApp=read(root,'js/endless-run-app.mjs');requireTokens(endlessApp,['ЗАБЕГ ЗАВЕРШЁН','ЗАРАБОТАНО ЗОЛОТА','ИТОГОВАЯ МОЩЬ','НОВАЯ ИГРА','ГЛАВНОЕ МЕНЮ','scene_defeat.jpg','queueMicrotask(() => open(storedRun))','RPChessEndlessRun'],'Endless Run final summary');
  const endlessCss=read(root,'css/endless-run.css');requireTokens(endlessCss,['.endless-run-screen','.endless-run-metrics',"'BrahmsGotischCyr'",'.endless-run-metric--power'],'Endless Run summary CSS');
  ...
  const rating=read(root,'js/player-rating.mjs');requireTokens(rating,["PLAYER_RATING_STORAGE_KEY = 'rpchess.reboot.v1.player-rating'",'STARTING_POWER = 500','ELO_K = 32','threatStarsForPower','expectedScore','settlePlayerRating','adaptiveEncounterStars','opponentEloForStars'],'Power/Elo core');
  const ratingRuntime=read(root,'js/player-rating-runtime.mjs');requireTokens(ratingRuntime,['settleCombat','settlePuzzle','data-power-result','rpchess:run-updated','RPChessPower','css/player-rating.css?v=20260830-power-1'],'Power result runtime');
  const ratingCss=read(root,'css/player-rating.css');requireTokens(ratingCss,['.travel-player-rating','.power-result','.power-result__delta.is-positive','.power-result__threat'],'Power/Threat CSS');
  const travel=read(root,'js/travel-choice-core.mjs');requireTokens(travel,['PLAYABLE_TRAVEL_TYPES','skirmish','battle','settlement','event','puzzle','adaptiveEncounterStars','threatStarsForPower','playerPower=STARTING_POWER','difficultyModel:\'power-v1\'','playerColor','enemyRaceTag'],'Power-adaptive Travel core');
  if(travel.includes('puzzleStarsForWeek(step)'))fail('Travel must not derive live Puzzle difficulty from journey week');
  const travelApp=read(root,'js/travel-choice-app.mjs');requireTokens(travelApp,['rpchess:event-open','rpchess:puzzle-open','СЛОЖНОСТЬ ЗАДАЧИ','Неделя путешествия','СЛУЧАЙНЫЙ БОЕЦ ПОГИБНЕТ','starsText','data-travel-power','data-travel-player-threat','readPlayerRating','difficultyModel===\'power-v1\''],'Power Travel runtime');
  for(const forbidden of ['ИСХОД НЕИЗВЕСТЕН','3–5 РЕШЕНИЙ','ВЫБРАТЬ ПУТЬ','Каждая карточка выбирается независимо','Куда двигаться дальше?','Шаг путешествия','СЛУЧАЙНАЯ СЛОЖНОСТЬ'])if(travelApp.includes(forbidden))fail(`Travel UX still contains removed copy: ${forbidden}`);
  ...
  const puzzleCore=read(root,'js/puzzles/puzzle-core.mjs');requireTokens(puzzleCore,['DIFFICULTY_TABLE','puzzleStarsForWeek','puzzleBaseGold','puzzleGoldReward','selectPuzzle','resolvedStars = clampStars(stars)','Puzzle catalog has no tasks for difficulty','isPuzzleState','applyPuzzleReward','ВЫИГРАЙТЕ'],'Power-adaptive Puzzle core');
  ...
  const puzzleApp=read(root,'js/puzzles/puzzle-app.mjs');requireTokens(puzzleApp,['ClassicChessEngine','rpchess:puzzle-open','Неверный ход','errors >= 3','rewardSettled','resumeSession','playForcedReply','Восстанавливаем ответ соперника…','Продолжить путь','Lichess Open Database · CC0','stars: choice.stars'],'Puzzle scene');
  ...
  const persistence=read(root,'js/run-persistence.mjs');requireTokens(persistence,['isPuzzleState','currentPuzzle','lastPuzzle','runStats:emptyRunStats()','hydrateRunStats','accrueRunStats','previousStoredRun'],'Endless Run persistence');
  ...
  const eventsV3=read(root,'js/events/event-content-v3.mjs');requireTokens(eventsV3,['EVENT_CONTENT_V3','ROLE_FALLBACK','applyEventContentV3','formatHeroReaction','Ваша Пешка','Ваш Конь','Ваш Слон','Ваша Ладья','Ваш Ферзь'],'Events v3 content overlay');
  const eventsApp=read(root,'js/events-app.mjs');requireTokens(eventsApp,['dataset.eventsScreen','events-outcome-modal','eventBackgroundPath','literaryStory','applyEventContentV3','formatHeroReaction','data-events-king-reaction','data-events-background','new URL(assetPath, document.baseURI).href','css/events.css?v=20260830-events-v3','rpchess:event-open','RPChessEvents','ИТОГИ ЗАБЕГА','RPChessEndlessRun?.open?.(current)'],'Events v3 UI explicit backdrop and Endless Run handoff');
  if(eventsApp.includes("setProperty('--events-background'"))fail('Events UI still uses legacy CSS-variable background injection');
  const eventsCss=read(root,'css/events.css');requireTokens(eventsCss,['.events-backdrop','.events-backdrop img',"'BrahmsGotischCyr'",'.events-reaction--king','.events-choice__reaction','.events-outcome-modal','@media(max-width:760px)'],'Events v3 CSS explicit backdrop and voice layers');
  ...
  const starvationApp=read(root,'js/starvation-app.mjs');requireTokens(starvationApp,['КОРОЛЬ ПОГИБ ОТ ГОЛОДА','ИТОГИ ЗАБЕГА','RPChessEndlessRun?.open?.(current)'],'Starvation Endless Run handoff');
  ...
  for(const cssPath of ['css/roster.css','css/skirmish.css','css/battle.css','css/travel-choice.css','css/player-rating.css','css/resources.css','css/settlement.css','css/starvation.css','css/events.css','css/puzzles.css','css/ux-consistency.css','css/endless-run.css']){

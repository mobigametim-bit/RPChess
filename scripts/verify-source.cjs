...
module.exports=function verifySource(root){
  ...
  for(let i=1;i<=5;i++)required.push(`js/events/event-data-v4-${String(i).padStart(2,'0')}.mjs`);
  for(let i=1;i<=25;i++)required.push(`js/events/event-data-v4c-${String(i).padStart(2,'0')}.mjs`);
  ...
  const route=read(root,'js/battle-route.mjs');requireTokens(route,["import './player-rating-runtime.mjs'","import './resources-app.mjs'","import './battle-app.mjs'","import './settlement-app.mjs'","import './starvation-app.mjs'","import './events-app.mjs'","import './puzzles/puzzle-app.mjs'","import './travel-choice-app.mjs'","import './ux-consistency.mjs'"],'journey bootstrap');
  ...
  const travelCss=read(root,'js/../css/travel-choice.css');requireTokens(travelCss,["'BrahmsGotischCyr'",'.travel-choice-heading h1'],'Travel fantasy typography');
  ...
  const eventsData=read(root,'js/events-data.mjs');requireTokens(eventsData,['EVENTS_01','EVENTS_10','EVENTS_V4_01','EVENTS_V4C_25','500 events / 2114 choices','EVENT_CATALOG','EVENT_IDS','normalizeRaceTag'],'Events expanded catalog index');
  ...
  const eventsCore=read(root,'js/events-core.mjs');requireTokens(eventsCore,['EVENT_COUNT = 500','shuffledEventIds','resolveEventChoice','event_king','markEventCombatStarted','eventCombatCompleted','clampStars','combatTheme'],'Events expanded core');
  const eventsBuilder=read(root,'js/events/event-data-v4-builder.mjs');requireTokens(eventsBuilder,['makeEvents','makeEvents3','expandEffect','Дварфы','Драконорождённые','dragonborn','dwarves'],'Events v4 content builder');
  ...
  for(const cssPath of ['css/roster.css','css/skirmish.css','css/battle.css','css/travel-choice.css','css/player-rating.css','css/resources.css','css/settlement.css','css/starvation.css','css/events.css','css/puzzles.css','css/ux-consistency.css']){

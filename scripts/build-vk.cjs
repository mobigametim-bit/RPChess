...
const HOSTING_EXCLUDED_EXTENSIONS = new Set(['.mp3', '.wav', '.ogg', '.m4a', '.aac', '.flac', '.mp4', '.webm', '.mov', '.txt']);

function requireFile(file, message) {
  if (!fs.existsSync(file) || !fs.statSync(file).isFile()) throw new Error(message || `Missing file: ${file}`);
}

function removeHostingExcludedFiles(root) {
  for (const entry of fs.readdirSync(root, { withFileTypes:true })) {
    const target = path.join(root, entry.name);
    if (entry.isDirectory()) {
      removeHostingExcludedFiles(target);
      continue;
    }
    if (HOSTING_EXCLUDED_EXTENSIONS.has(path.extname(entry.name).toLowerCase())) fs.unlinkSync(target);
  }
}
...
function main() {
  ...
  removeHostingExcludedFiles(VK_DIST);

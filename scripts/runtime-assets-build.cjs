...
const brandLogo=require('./brand-logo-asset-runtime.cjs');
...
const BRAND_LOGO_FILE=require.resolve('./brand-logo-asset-runtime.cjs');
...
function materializeBrandLogos(root){
  const version=fingerprintFiles([ORCHESTRATOR_FILE,PIECE_FILE,BRAND_LOGO_FILE]),records=[];
  for(const item of brandLogo.inspectBrandLogoAssets(root).records){
    const full=path.join(root,item.path),source=fs.readFileSync(full),before=source.length;
    const cached=cachedPng(source,{namespace:'brand-logos',version,maxSide:brandLogo.BRAND_LOGO_RUNTIME_MAX_SIDE,maxBytes:brandLogo.BRAND_LOGO_RUNTIME_MAX_BYTES,validateExtra:(png)=>{if(png.width<=png.height)throw new Error('cached brand logo is not landscape');if(![3,4,6].includes(png.colorType))throw new Error(`cached brand logo lost transparency capability: colorType=${png.colorType}`);}});
    const out=piece.parsePng(cached.buffer);fs.writeFileSync(full,cached.buffer);
    records.push({path:item.path,before,after:cached.buffer.length,width:out.width,height:out.height,sourceWidth:item.width,sourceHeight:item.height,skipped:false,cacheHit:cached.cacheHit,cacheKey:cached.cacheKey,cachePath:cached.cachePath});
  }
  const result=report(records);if(result.afterBytes>brandLogo.BRAND_LOGO_RUNTIME_MAX_TOTAL_BYTES)throw new Error(`[brand logo asset budget] aggregate ${piece.formatBytes(result.afterBytes)} exceeds ${piece.formatBytes(brandLogo.BRAND_LOGO_RUNTIME_MAX_TOTAL_BYTES)}`);return result;
}
...
function materializeRuntimeAssets(root){
  const groups={
    ...
    brandLogos:materializeBrandLogos(root),

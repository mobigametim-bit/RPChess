...
| TUTORIAL-001 `assets/tutorial/chapters/chapter_01.jpg`…`chapter_24.jpg` | 24 × 1600×900 | Clean instructional chess scenes, one concept per image, large readable pieces, uncluttered modular-cell board, no embedded text/arrows that require localization. | concept readable; RU/EN overlays fit; no false legal position; displayed cell pair matches an approved board theme | MISSING |
...
| COSMETIC-001 `assets/cosmetics/board_tiles/<theme>/{tile_light.png,tile_dark.png}`; `assets/cosmetics/{pieces,banners}/` | cosmetic board themes are paired 512×512 cells; additional piece/banner reward sets | Cosmetic-only fantasy variants preserving classic silhouettes and legal-state readability. Every board theme consists only of one top-down light cell and one matching dark cell; full bleed, calm center, no complete-board image, no board frame, no underlay, no coordinates, no baked highlights or objects. | no gameplay advantage; paired cells share scale/light direction; parity readable at 64 px; figures and highlights pass contrast tests; arbitrary board configurations assemble without seams | MISSING |
...
| EDITOR-001 `assets/editor/{tools,objects,objectives,validation}.png` | 4 sheets × 16 icons | Clear visual editor icon family for logical board shape, cells, pieces, objects, portals, objectives, phases, validation and test play. | understandable at 32 px; controller focus states | MISSING |

### Cosmetic board-theme rule

Cosmetic board rewards never replace gameplay geometry and never require a complete board raster. A theme is approved only as a `tile_light.png`/`tile_dark.png` pair using the same `BOARD-TILE-512` rules as regional surfaces. The game supplies coordinates, active-cell masks and all state overlays. Board frames and decorative underlays are outside the production scope.

Complete board illustrations may still be created for Steam art, Chronicle scenes or promotional material, but they are scene compositions rather than runtime board cosmetics.

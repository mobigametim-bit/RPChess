...
(async () => {
  ...
  try {
    ...
    await page.waitForFunction(() => window.RPChessClassicChess.moveLog.length >= 2 && !window.RPChessChessAI.thinking, null, { timeout: 20000 });
    ...
    await page.waitForFunction(() => window.RPChessClassicChess.moveLog.length >= 1 && !window.RPChessChessAI.thinking, null, { timeout: 20000 });
    ...
    console.log('Classic Chess + real Stockfish Chromium acceptance: PASS');
  } finally {
    await browser.close();
  }
})().catch((error) => {

...
(async () => {
  ...
  assert.strictEqual(registry.CONTENT_REGISTRY.adapters.events.catalog.length, 500);
  assert.strictEqual(registry.CONTENT_REGISTRY.adapters.events.catalog.reduce((sum, event) => sum + event.choices.length, 0), 2114);
  assert.strictEqual(registry.CONTENT_REGISTRY.adapters.events.expectedEvents, 500);

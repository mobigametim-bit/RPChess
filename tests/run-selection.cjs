...
test('selection catalog exposes implemented Iron Marches choices with canonical Register 01 art', () => {
  ...
  assert.strictEqual(catalog.kings[0].assets.portrait, 'assets/kings/oathkeeper/portrait.png');
  assert.strictEqual(catalog.doctrines[0].assets.emblem, 'assets/doctrines/fortress/emblem.png');
  assert.ok(catalog.heroes.every((hero) => hero.id.startsWith('hero.')));
});

test('king, doctrine and heroes use immutable deterministic revisions', () => {
...
test('selection enforces roster limits and required choices before lock', () => {
...
test('selecting a new king clears an incompatible doctrine', () => {
  const records = {
...
test('locked selection survives snapshot restoration', () => {
...
test('selection presenter localizes content and exposes compatibility', () => {
  ...
  assert.strictEqual(presenter.selectedKing.assets.portrait, 'assets/kings/oathkeeper/portrait.png');
  assert.strictEqual(presenter.selectedDoctrine.assets.emblem, 'assets/doctrines/fortress/emblem.png');
...
test('locked setup is passed into deterministic Iron Marches bootstrap', () => {

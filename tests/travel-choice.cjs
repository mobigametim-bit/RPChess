...
(async () => {
  ...
  assert.deepStrictEqual(travel.PLAYABLE_TRAVEL_TYPES, ['skirmish', 'battle']);
  ...
  assert(first.some((choice) => choice.type === 'skirmish'), 'v1 fork must contain a Skirmish path');
  assert(first.some((choice) => choice.type === 'battle'), 'v1 fork must contain a Battle path');
  ...
  const chosen = { ...first[0], combatCountAtSelection: 0 };
  ...
  console.log('Travel Choice deterministic offers, flavor variation, persistence and encounter routing contract: PASS');
})().catch((error) => {

...
(async()=>{const browser=await chromium.launch({headless:true});try{
  ...
  await clickMove(page,moves.correct.from,moves.correct.to,moves.correct.promotion);await page.locator('[data-puzzle-outcome]:not([hidden])').waitFor();assert.strictEqual((await page.locator('[data-puzzle-outcome-title]').innerText()).trim(),'РЕШЕНО');assert((await page.locator('[data-puzzle-outcome-gold]').innerText()).includes('+8 Gold'),'★1 with one error must pay rounded 70% of 12 Gold');

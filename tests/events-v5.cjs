...
(async()=>{
  ...
  assert(app.includes("state:'missing', locked:true")&&app.includes("if(heroState.state==='missing')return '🔒';")&&app.includes('choice.heroLine'),'missing hero upper strip must preserve semantic missing state, render lock-only and keep the hero line present');

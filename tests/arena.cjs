...
(async()=>{
  ...
  const clockSkew=Date.now()+60_000;
  const earlierReward=core.addEvent(core.emptyArena(),{id:'clock:earlier:win',kind:'win',foe:foes[0].id,amount:200,at:Date.now()-3000});
  const alreadyBought=core.purchaseSquad(earlierReward,'elves','clock:elves');
  const earnedOnOtherDevice=core.addEvent(alreadyBought,{id:'clock:later:win',kind:'win',foe:foes[0].id,amount:98,at:clockSkew});
  assert.strictEqual(core.arenaSummary(earnedOnOtherDevice).balance,178);
  const thirdSquad=core.purchaseSquad(earnedOnOtherDevice,'orcs','clock:orcs');
  assert(core.arenaSummary(thirdSquad).owned.has('orcs'),'affordable third squad stays purchased even when rewards came from a clock-ahead device');
  assert.strictEqual(core.arenaSummary(thirdSquad).balance,18);
  assert.strictEqual(core.arenaSummary(core.normalizeArena(JSON.parse(JSON.stringify(thirdSquad)))).balance,18,'purchase survives reload');

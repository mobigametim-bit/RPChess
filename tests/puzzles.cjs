...
(async()=>{
  ...
  const mate1=catalog.find(p=>p.type==='mate1');const mateEngine=new ClassicChessEngine(mate1.fen);const legalMates=mateEngine.legalMoves().filter(move=>{const test=new ClassicChessEngine(mate1.fen);const result=test.move(move.from,move.to,move.promotion);return result.ok&&result.status.type==='checkmate'&&result.status.winner===mate1.side;});assert(legalMates.length>=1,'mate1 must expose at least one legal mating move');

  for(let stars=1;stars<=12;stars++){const a=core.selectPuzzle(catalog,{runId:'stable',routeId:`route-${stars}`,stars,week:(stars-1)*8+1}),b=core.selectPuzzle(catalog,{runId:'stable',routeId:`route-${stars}`,stars,week:(stars-1)*8+1});assert.strictEqual(a.id,b.id,'selection must be deterministic');assert.strictEqual(a.difficulty,stars,'selection should stay in current star pool when available');}
  const appSource=fs.readFileSync(path.join(game,'js/puzzles/puzzle-app.mjs'),'utf8');assert(appSource.includes("rpchess:puzzle-open"));assert(!/hint/i.test(appSource),'Puzzles v1 must not introduce hint UI/mechanics');assert(appSource.includes("'Неверный ход'"));assert(appSource.includes('errors>=3'));assert(appSource.includes('rewardSettled'));
  console.log(`Puzzles difficulty/reward/catalog/solution/runtime contracts: PASS (${catalog.length} seed puzzles)`);

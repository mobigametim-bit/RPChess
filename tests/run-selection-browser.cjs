...
(async () => {
  ...
  test('local selection host reaches a ready deterministic runtime only after lock', async () => {
    ...
    assert.throws(() => host.dispatch({ type: 'ToggleHero', heroId: 'hero.mara_chain' }), /already launched/);
  });

  test('browser client emits ready with the runtime snapshot and serializes pending commands', async () => {
  ...
  test('selection markup is escaped, accessible and disables incompatible doctrine cards', () => {
  ...
  test('snapshot validator rejects ready state without a runtime handoff', () => {
